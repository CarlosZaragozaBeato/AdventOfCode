...
the bag contained only 12 red cubes,
13 green cubes, and 14 blue cubes?

In the example above, games 1, 2, and 5 would have
been possible if the bag had been loaded with that configuration.
...
func main() {
	var arrayData []map[string]int
	var arrayIds []int
	res_red := 12
	res_green := 13
	res_blue := 14

	file, _ := os.Open("/home/carlos/data/AdventOfCode/02/Go/file.txt")
	defer file.Close()
	...
	for scanner.Scan() {
		...
		linea := scanner.Text()
		first_split := strings.Split(linea, ":")
		id := strings.Split(first_split[0], " ")[1]
		myDict["id"], _ = strconv.Atoi(id)
		second_split := strings.Split(first_split[1], ",")

		for _, cube := range second_split {
			if strings.Contains(cube, ";") {
				lst_cube_temp := strings.Split(cube, ";")
				for _, cube_temp := range lst_cube_temp {
					cube := strings.Split(cube_temp[1:], " ")
					_, encontrado := myDict[cube[1]]
					cube_valor, _ := strconv.Atoi(cube[0])
					if encontrado {
						myDict[cube[1]] += cube_valor
					} else {
						myDict[cube[1]] = cube_valor
					}
				}
			} else {
				cube := strings.Split(cube[1:], " ")
				_, encontrado := myDict[cube[1]]
				cube_valor, _ := strconv.Atoi(cube[0])
		...
		arrayData = append(arrayData, myDict)
	}

	for _, game := range arrayData {
		fmt.Println(game)
		game_red, _ := game["red"]
		game_blue, _ := game["blue"]
		game_green, _ := game["green"]

		if game_red <= res_red && game_blue <= res_blue && game_green <= res_green {
			id, _ := game["id"]
