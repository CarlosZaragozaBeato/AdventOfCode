...
if __name__ == "__main__":
    ...
    with open("/home/carlos/data/AdventOfCode/02/Python/file.txt", "r") as f: 
        lines = f.readlines() 
        ...
        for line in lines:
            ...
            # ID FORMAT 
            id = first_split[0].split("e")[1]

            # COLORS FORMAT 
            second_split = first_split[1].replace("\n", "").split(",")
            ...
            for index, cube in enumerate(second_split:
                if ";" in  cube:
                    semi_cube = cube.split(";")
                    for cube_temp in semi_cube:
                        cube_temp = cube_temp[1:]
                        split_cube_temp = cube_temp.split(" ")
                        if split_cube_temp[1] in dict_temp.keys():
                            dict_temp[split_cube_temp[1]] += int(split_cube_temp[0])
                        else:
                            dict_temp[split_cube_temp[1]] = int(split_cube_temp[0])
    ...
    for game in data: 
        if game["red"] <= res_red and game["green"] <= res_green and game["blue"] <= res_blue:
            valid_id_games.append(game["id"])
        print(game)
        print("----")
    print(valid_id_games)
